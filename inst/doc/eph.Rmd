...
- `year`: un valor numérico entero con el año de la encuesta que se desea descargar
- `trimester`: un valor numérico entero con el trimestre (este argumento es válido para la EPH continua, 2004-actualidad)
- `type`: un valor character `"individual"`, para descargar la base de invidividuos y `"hogar"`, para la de hogar.
...
- `add.totals`: agrega maginales por fila (`row`), columnas (`col`) o ambas (`both`)
- `add.percentage`: calcula porcentajes por fila (`row`), columnas (`col`) o ambas (`both`)
...
- `variables`: string con los nombres de las variables de interés
